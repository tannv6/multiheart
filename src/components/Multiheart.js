...
function Multiheat() {
    ...
    useEffect(() => {
        ...
        var ParticleConstructor = (function () {
            function Particle() {
                this.position = new PointConstructor();
                this.velocity = new PointConstructor();
                this.acceleration = new PointConstructor();
                this.age = 0;
            }
        ...
        var ParticlePoolConstructor = (function () {
            var particles,
                firstActive = 0,
                firstFree = 0,
            ...
            function ParticlePool(length) {
                ...
                for (var i = 0; i < particles.length; i++)
                    particles[i] = new ParticleConstructor();
            }
        ...
        (function (canvas) {
            var context =
                ...
                particles = new ParticlePoolConstructor(settings.particles.length),
        ...
        let count = 0;
        let countTwo = heading.children.length;
        let timer = null;
        setTimeout(myFunc, 1000);
        function myFunc() {
            let name = heading.children;
            timer = setInterval(() => {
                runFunc();
            }, 1200);
            function runFunc() {
                if (count === name.length - 1 || countTwo === 1) {
                    clearInterval(timer);
                    timer = null;
                    setTimeout(() => {
                        timer = setInterval(() => {
                ...
                if (count < name.length) {
                    name[count].className = "animationOne";
                    count++;
                } else {
                    name[countTwo - 1].className = "animationTwo";
                    countTwo--;
                    if (countTwo <= 0) {
                        count = 0;
                        countTwo = name.length;
                    }
                }
            }
        }
